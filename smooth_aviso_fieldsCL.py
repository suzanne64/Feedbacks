...
class smooth_params:
    def __init__(self,args):
        ...
        yf,mf,df = [int(item) for item in args.lastdate.split('-')]
...
#    print('\n'.join("%s: %s" % item for item in attrs.items()))
    ...
    filelist=sorted(glob.glob(data_path['data_dir'] + 'dt_global_allsat_msla_h*.nc'))
...
#    plt.title('Sea level anomaly');plt.show()
    ...
    if os.path.isfile(file_wts):
        ...
        filename = data_path['data_dir'] + 'smooth_wts_avisogrid_fwhm{0:d}.nc'.format(int(params.fwhm))
        print(filename)
